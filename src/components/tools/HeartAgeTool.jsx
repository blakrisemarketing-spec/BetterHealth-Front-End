...
import { heartShareSpec } from "../../data/tools/share-card";
import ToolCta from "./ToolCta";
import ShareResult from "./ShareResult";
import HeartHabitsSummary from "./HeartHabitsSummary";
...
/** Everything under the headline card, shared by all three result kinds. */
function Below({ result, tool, panel, children }) {
  return (
    <RevealAfter delay={1.3}>
      <ShareResult spec={heartShareSpec(result)} />
      <HeartHabitsSummary habits={result.habits} />
      {children}
      ...
      <div>
        <ResultCard>
          ...
          <RevealAfter delay={0.5}>
            <p className="text-[15px] text-text-secondary leading-relaxed mb-3">
              Blood pressure is one of the four things the chart is built on, and it is the only one you cannot
              estimate from how you feel. Around one in three Ghanaian adults has high blood pressure and most of them
              do not know it, because it rarely causes any symptom until something goes wrong.
            </p>
            <p className="text-[15px] text-text-secondary leading-relaxed mb-3">
        ...
        <Below result={result} tool={tool} panel={panel} />
      </div>
    );
  }
  ...
  if (result.kind === "outOfRange") {
    return (
      <div>
        <ResultCard>
          <span className="block text-[12px] font-bold text-primary uppercase tracking-[0.12em] mb-1">
            Part 1: outside the published range
          </span>
          <h2 className="text-[1.4rem] sm:text-[1.8rem] font-extrabold text-text-primary font-heading leading-tight mb-3">
          ...
          <RevealAfter delay={0.5}>
            <p className="text-[15px] text-text-secondary leading-relaxed mb-3">
              We could stretch the numbers to cover you. We are not going to, because a figure invented past the edge
              of a table is worse than no figure at all.
            </p>
            <p className="text-[15px] text-text-secondary leading-relaxed">
  ...
  return (
    <div>
      <ResultCard>
        ...
        <BandMeter bands={RISK_BANDS} activeId={band.id} leftLabel="Under 5%" rightLabel="30% or above" />

        <RevealAfter>
          <p className="text-[12px] font-bold text-text-secondary uppercase tracking-[0.1em] mb-2">Your heart age</p>
          {heartAge.ageKey ? (
            <p className="text-[15px] text-text-secondary leading-relaxed mb-4">
              {sameAsAge ? (
                ...
                <>
                  ...
                  band at <strong className="text-text-primary">{AGE_LABELS[heartAge.ageKey]}</strong>. That is what
                  the phrase heart age means here, and it is read off the same table, not worked out from a second
                  formula.
                </>
              )}
            </p>
          ...
          {drivers.length === 0 ? (
            <p className="text-[15px] text-text-secondary leading-relaxed mb-4">
              Nothing. On smoking, blood pressure and BMI you are already in the chart&rsquo;s healthiest column, so
              what is left is your age.
            </p>
          ) : (
            <>
              ...
              <p className="text-[12.5px] text-text-muted leading-relaxed mb-4">
                Each figure is the gap in percentage points between your own cell on the chart and the cell you would
                sit in with that one factor at its healthiest published value. Both are printed numbers.
              </p>
            </>
          )}
          ...
          {bmi && (
            <p className="text-[13px] text-text-secondary mb-4">
              Your BMI works out at <strong className="text-text-primary">{bmi.toFixed(1)}</strong>, which is the
              chart&rsquo;s {BMI_LABELS[bmiIndex].toLowerCase()} column.
            </p>
          )}
          ...
          <div className="rounded-card border-l-4 border-primary bg-primary-bg px-4 py-3 text-[14px] text-text-primary leading-relaxed">
            ...
            smoking status, blood pressure and BMI. It is an estimate drawn from population data rather than a
            statement about you.
          </div>
        </RevealAfter>
      </ResultCard>
